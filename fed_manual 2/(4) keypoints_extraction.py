...
#-----------------------------CONCEPT DATA SET FOR TCAV----------------------------------------------------

def build_concept_dataset(df_features, split_dir: Path, angle_threshold=None):
    """
    Construye el dataset de conceptos para TCAV a partir de las features
    ya calculadas por process_implant(). Copia los crops LIMPIOS
    (sin anotaciones de matplotlib) a:
        split_dir/concepts/<concepto>/positive/
        split_dir/concepts/<concepto>/negative/

    angle_threshold: si se pasa (calculado en train), se reutiliza en
    val/test para que las tres particiones usen el mismo criterio.
    Devuelve el threshold usado (para propagarlo desde train).
    """
    images_dir   = split_dir / "cropped_images"
    concepts_dir = split_dir / "concepts"

    # ---- Concepto 1: tipo_implante (U vs Recto) ----
    mapping = [("Tipo U", "tipo_implante_U"), ("Tipo Recto", "tipo_implante_Recto")]
    for concept_value, concept_name in mapping:
        pos_dir = concepts_dir / concept_name / "positive"
        neg_dir = concepts_dir / concept_name / "negative"
        pos_dir.mkdir(parents=True, exist_ok=True)
        neg_dir.mkdir(parents=True, exist_ok=True)

        for _, row in df_features.iterrows():
            src = images_dir / f"{row['image_name']}.png"
            if not src.exists():
                continue
            dst_dir = pos_dir if row["tipo_implante"] == concept_value else neg_dir
            shutil.copy2(src, dst_dir / src.name)

    # ---- Concepto 2: steep_thread_angle (ángulo de rosca pronunciado) ----
    valid = df_features.dropna(subset=["angle_left_valley", "angle_right_valley"]).copy()
    valid["mean_abs_angle"] = (
        valid["angle_left_valley"].abs() + valid["angle_right_valley"].abs()
    ) / 2

    if angle_threshold is None:
        angle_threshold = float(valid["mean_abs_angle"].median())

    concept_name = "steep_thread_angle"
    pos_dir = concepts_dir / concept_name / "positive"
    neg_dir = concepts_dir / concept_name / "negative"
    pos_dir.mkdir(parents=True, exist_ok=True)
    neg_dir.mkdir(parents=True, exist_ok=True)
    ...
    for _, row in valid.iterrows():
        ...
        if not src.exists():
            continue
        dst_dir = pos_dir if row["mean_abs_angle"] > angle_threshold else neg_dir
        shutil.copy2(src, dst_dir / src.name)

    print(f"  Concept dataset -> '{concepts_dir}'  (angle_threshold = {angle_threshold:.2f}°)")
...
def main():
  ...
  # features of all images
    ...
    train_angle_threshold = None   # <-- añadir antes del for
    ...
    for split in splits:
        ...
        # --- Paso 7: dataset de conceptos para TCAV ---   <-- NUEVO
        if split.name == "train":
            train_angle_threshold = build_concept_dataset(df_features, split)
        else:
            build_concept_dataset(df_features, split, angle_threshold=train_angle_threshold)
